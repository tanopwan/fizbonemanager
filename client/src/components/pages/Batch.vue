<template>
  <div>
      ...
      <div class="col-md-6 col-xs-12">
        <div class="block">
          <div class="row form-group" :class="{ 'has-error': quantityError }">
            ...
            <div class="col-sm-6">
              <select class="select-select2" style="width: 100%;" data-placeholder="Choose Product..">
                <option></option>
      ...
      <div class="col-xs-12">
        <div class="block full">
          <div class="block-title">
            <h4>
              All Batches
              <small> Total: {{ batches.length }}</small>
            </h4>
          </div>
            ...
            <tbody>
              <tr v-for="batch in batches" v-bind:key="batch._id" :class="{success: !batch.isFinish, active: batch.isFinish}">
                ...
                <td class="text-center">
                  ...
                  <a href="#create-expense-modal" @click="showExpenseModal(batch._id)" data-toggle="modal" class="btn btn-effect-ripple btn-sm btn-warning" style="overflow: hidden; position: relative;">
                    <i class="fa fa-credit-card"></i>
                  </a>
                  <a href="javascript:void(0)" @click="deleteBatch(batch._id)" style="overflow: hidden; position: relative;">
    ...
    <div id="create-expense-modal" class="modal" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
          ...
          <div class="modal-body">
            <div class="row form-group">
              <div class="col-sm-6">
                <date-picker update="false" v-model="date"></date-picker>
              </div>
              <div class="col-sm-3">
                <select2 :options="monthOptions" v-model="expenseMonth" allowClear="false" class="form-control"></select2>
              </div>
              <div class="col-sm-3">
                <input type="text" v-model="expenseYear" class="form-control" placeholder="2017">
              </div>
            </div>
            <div class="row form-group">
              <div class="col-sm-6">
                <div class="input-group">
                  ...
                  <input type="text" v-model="expenseTitle" class="form-control" placeholder="รายการ">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="input-group">
                  <span class="input-group-addon">ราคา</span>
                  <input type="text" v-model="expenseAmount" class="form-control">
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="modal-footer">
          <i v-if="saving" class="fa fa-asterisk fa-2x fa-spin text-success"></i>
          <div v-else>
            <button type="button" @click="createExpense" class="btn btn-effect-ripple btn-success" style="overflow: hidden; position: relative;">Create</button>
            <button type="button" class="btn btn-effect-ripple btn-danger" data-dismiss="modal" style="overflow: hidden; position: relative;">Close</button>
          </div>
        </div>
      </div>
...
export default {
  data() {
    return {
      ...
      expenseTitle: "",
      expenseAmount: 0,
      expenseMonth: moment().month(),
      expenseYear: moment().year(),
      monthOptions: [
        { text: "Jan", id: 0 },
        { text: "Feb", id: 1 },
        { text: "Mar", id: 2 },
  ...
  methods: {
    addBatch() {
      ...
      this.$http.post("/api/batches", body).then(
        response => {
    ...
    deleteBatch(id) {
      this.$http.delete("/api/batches/" + id).then(
        response => {
          let index = -1;
    ...
    setIsFinish(id, isFinish) {
      this.$http.put(`/api/batches/${id}/finish/${isFinish}`).then(
        response => {
          this.batches.forEach((batch, idx) => {
            if (batch._id === id) {
